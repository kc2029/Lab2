function start() {
  ...
  document.getElementById("nbBees").innerHTML = 0;
  clearTimeout();
  //create bear
  bear = new Bear();
  ...
  bees = new Array();
  console.log("bee array", bees);
  makeBees();
  updateBees();
  lastStingTime = new Date().addEventListener("keydown");
  //document.addEventListener("keydown", (lastStingTime = new Date()), true);
  console.log("start timer", lastStingTime);
}
...
function isHit(defender, offender) {
  if (overlap(defender, offender)) {
    ...
    console.log("duration.innerHTML:", duration.innerHTML);
    //calculate longest duration
    let newStingTime = new Date();
    let thisDuration = newStingTime - lastStingTime;
    lastStingTime = newStingTime;
    let longestDuration = Number(duration.innerHTML); // if duration is ?? => number of ?? => NaN
    if (!longestDuration) {
      // check if valid number (not just zero)
